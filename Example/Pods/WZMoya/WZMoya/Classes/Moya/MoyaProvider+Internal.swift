...
public extension MoyaProvider {
    ...
    func requestNormal(_ target: Target, callbackQueue: DispatchQueue?, progress: WZMoya.ProgressBlock?, completion: @escaping WZMoya.Completion) -> Cancellable {
        let endpoint = self.endpoint(target)
        let stubBehavior = self.stubClosure(target)
        let cancellableToken = CancellableWrapper()

        // Allow plugins to modify response
        let pluginsWithCompletion: WZMoya.Completion = { result in
        ...
        let performNetworking = { (requestResult: Result<URLRequest, MoyaError>) in
            ...
            let networkCompletion: WZMoya.Completion = { result in
    ...
    private func performRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, progress: WZMoya.ProgressBlock?, completion: @escaping WZMoya.Completion, endpoint: Endpoint, stubBehavior: WZMoya.StubBehavior) -> Cancellable {
        switch stubBehavior {
        case .never:
            switch endpoint.task {
            case .requestPlain, .requestData, .requestJSONEncodable, .requestCustomJSONEncodable, .requestParameters, .requestEncryption, .requestCompositeData, .requestCompositeParameters:
    ...
    func cancelCompletion(_ completion: WZMoya.Completion, target: Target) {
    ...
    final func createStubFunction(_ token: CancellableToken, forTarget target: Target, withCompletion completion: @escaping WZMoya.Completion, endpoint: Endpoint, plugins: [PluginType], request: URLRequest) -> (() -> Void) { // swiftlint:disable:this function_parameter_count
        return {
            ...
            let validate = { (response: WZMoya.Response) -> Result<WZMoya.Response, MoyaError> in
            ...
            case .networkResponse(let statusCode, let data):
                let response = WZMoya.Response(statusCode: statusCode, data: data, request: request, response: nil)
                let result = validate(response)
                plugins.forEach { $0.didReceive(result, target: target) }
                completion(result)
            case .response(let customResponse, let data):
                let response = WZMoya.Response(statusCode: customResponse.statusCode, data: data, request: request, response: customResponse)
                let result = validate(response)
                plugins.forEach { $0.didReceive(result, target: target) }
                completion(result)
...
private extension MoyaProvider {
    ...
    func sendUploadMultipart(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, multipartBody: [MultipartFormData], progress: WZMoya.ProgressBlock? = nil, completion: @escaping WZMoya.Completion) -> CancellableToken {
    ...
    func sendRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, progress: WZMoya.ProgressBlock?, completion: @escaping WZMoya.Completion) -> CancellableToken {
        let interceptor = self.interceptor(target: target)
    ...
    func sendAlamofireRequest<T>(_ alamoRequest: T, target: Target, callbackQueue: DispatchQueue?, progress progressCompletion: WZMoya.ProgressBlock?, completion: @escaping WZMoya.Completion) -> CancellableToken where T: Requestable, T: Request {
