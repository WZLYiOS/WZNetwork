...
public typealias Completion = (_ result: Result<WZMoya.Response, MoyaError>) -> Void
...
public protocol MoyaProviderType: AnyObject {
    ...
    func request(_ target: Target, callbackQueue: DispatchQueue?, progress: WZMoya.ProgressBlock?, completion: @escaping WZMoya.Completion) -> Cancellable
}
...
open class MoyaProvider<Target: TargetType>: MoyaProviderType {
    ...
    public typealias StubClosure = (Target) -> WZMoya.StubBehavior
    ...
    open internal(set) var inflightRequests: [Endpoint: [WZMoya.Completion]] = [:]
    ...
    @discardableResult
    open func stubRequest(_ target: Target, request: URLRequest, callbackQueue: DispatchQueue?, completion: @escaping WZMoya.Completion, endpoint: Endpoint, stubBehavior: WZMoya.StubBehavior) -> CancellableToken {
        let callbackQueue = callbackQueue ?? self.callbackQueue
...
public extension MoyaProvider {
    ...
    /// Do not stub.
    final class func neverStub(_: Target) -> WZMoya.StubBehavior {
        return .never
    }

    /// Return a response immediately.
    final class func immediatelyStub(_: Target) -> WZMoya.StubBehavior {
        return .immediate
    }

    /// Return a response after a delay.
    final class func delayedStub(_ seconds: TimeInterval) -> (Target) -> WZMoya.StubBehavior {
...
public func convertResponseToResult(_ response: HTTPURLResponse?, request: URLRequest?, data: Data?, error: Swift.Error?) ->
    Result<WZMoya.Response, MoyaError> {
        switch (response, data, error) {
        case let (.some(response), data, .none):
            let response = WZMoya.Response(statusCode: response.statusCode, data: data ?? Data(), request: request, response: response)
